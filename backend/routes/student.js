...
// Exchange question set for student
router.post('/exam/:examId/exchange', authMiddleware, studentAuth, async (req, res) => {
  try {
    const { examId } = req.params;
    ...
    if (!studentExam.exchangeAllowed) {
      return res.status(403).json({ message: 'Exchange not allowed by teacher' });
    }
    ...
    // Ensure at least one question was selected
    if (selectedQuestions.length === 0) {
        return res.status(400).json({ message: 'Not enough questions available in the pool to generate a set.' });
    }
    ...
    // Update the student exam with new questions and increment exchange count
    studentExam.assignedQuestions = finalQuestions;
    studentExam.exchangeCount += 1;
    studentExam.exchangeAllowed = false; // Reset permission after exchange

    await studentExam.save();

    res.json({
      message: 'Question set exchanged successfully',
      questions: finalQuestions
    });
  } catch (error) {
    console.error('Error exchanging question set:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
router.get('/exam/:examId/status', authMiddleware, studentAuth, async (req, res) => {
  try {
    ...
    if (!studentExam) {
      return res.json({ status: 'not_started', setGenerated: false, uploadedScreenshots: {}, exchangeAllowed: false, exchangeCount: 0 });
    }
    ...
    res.json({
      ...
      exchangeCount: studentExam.exchangeCount,
    });
  } catch (error) {
